...
from .services.farmbox_plantio_sync import (
    fetch_farmbox_plantations_parallel,
    get_milliseconds_from_datetime,
)
...
def process_farmbox_plantations_payload(payload, safra_nome=None):
    """
    Reaproveita a mesma lógica da action update_plantio_from_farmBox,
    mas como função interna para poder ser chamada por outras actions.
    """

    BATCH_SIZE = 500

    if not isinstance(payload, (list, tuple)):
        raise ValueError("Payload deve ser uma lista.")
    ...
    ciclo_list = list(Ciclo.objects.all().only("id"))
    ...
    area_by_farmbox = {
        row["plantio__id_farmbox"]: (row["total_area_plantada"] or 0)
        for row in plantio_area_qs
    }

    plantios_by_farmbox = Plantio.objects.filter(
        id_farmbox__in=ids_farmbox_list
    ).in_bulk(field_name="id_farmbox")
    ...
    for it in payload:
        ...
        try:
            safra = safra_by_nome.get(safra_farm)
    ...
    if combo_keys:
        ...
        plantio_by_combo = {
            (p.safra_id, p.ciclo_id, p.talhao_id): p
            for p in plantios_combo_qs
        }
    ...
    for it, safra, ciclo, talhao in resolvidos_temp:
        try:
            ...
            var_planejada = (
                variedade_by_farmbox.get(variedade_planejada_id)
                if variedade_planejada_id
                else None
            )
            var_definitiva = (
                variedade_by_farmbox.get(variety_id)
                if variety_id
                else None
            )
            ...
            if not inst:
                if not (safra and ciclo and talhao):
                    ...
                    erros.append({
                        ...
                        "motivo": "Sem safra/ciclo/talhão para criar.",
                        "name": it.get("name"),
                        "farm_name": it.get("farm_name"),
                        "harvest_name": it.get("harvest_name"),
                    })
                    continue
    ...
    with transaction.atomic():
        ...
        if safra_nome:
            total_now = Plantio.objects.filter(safra__safra=safra_nome).count()
        else:
            payload_safras = {
                str(it.get("harvest_name")).strip()
                for it in payload
                if it.get("harvest_name")
            }

            if len(payload_safras) == 1:
                total_now = Plantio.objects.filter(
                    safra__safra=list(payload_safras)[0]
                ).count()
            else:
                total_now = Plantio.objects.count()
        ...
        resp = {
            ...
            "ignoradas": count_ignorados,

            # aliases novos
            "alteradas_com_farmbox_update": count_alterados,
...
class PlantioViewSet(viewsets.ModelViewSet):
        ...
        try:
            ...
            for i in new_list:
                try:
                    ...
                    fazenda_id = i["farm"]["id"]
                    safra_farm = i["harvest_name"]
                    safra = next(x for x in safra_list if x.safra == safra_farm)
                    ciclo_json = i["cycle"]
                    ciclo = ciclo_list[ciclo_json - 1]
                    cultura_planejada = i.get("planned_culture_name")
                    variedade_planejada_id = i.get("planned_variety_id")
                    id_plantio_farmbox = i["id"]
                    map_centro_id_farm = i["centroid"]
                    map_geo_points_farm = i["geo_points"]
                    area = i["area"]
                    area_planejamento = area

                    id_talhao = next((x.id_d for x in projetos if x.id_farmbox == fazenda_id), None)
                    talhao_id = None
                    if id_talhao:
                        talhao_unico = f"{id_talhao}{parcela}"
                        talhao_id = next((x for x in talhao_list if x.id_unico == talhao_unico), None)
                        if not talhao_id:
                            print(f"id sem cadastro: {talhao_unico}")

                    id_variedade = next((x for x in variedade_list if x.id_farmbox == variedade_planejada_id), None)
                    if not id_variedade and cultura_planejada:
                        print(f"Variedade não cadastrada: {variedade_planejada_id}")
                        continue
                    ...
                    if cultura_planejada:
                        if ciclo_json == 1:
                            ciclo_1[cultura_planejada] = ciclo_1.get(cultura_planejada, 0) + area
                        elif ciclo_json == 2:
                            ciclo_2[cultura_planejada] = ciclo_2.get(cultura_planejada, 0) + area
                        elif ciclo_json == 3:
                            ciclo_3[cultura_planejada] = ciclo_3.get(cultura_planejada, 0) + area

                except Exception as e:
                    print(f"❌ Erro ao salvar plantio ID {i.get('id')}: {e}")
                    continue  # ignora erro e vai pro próximo

            print("Resumo por ciclo:")
            for k, v in ciclo_1.items():
                print(f"Ciclo 1 - {k}: {round(v)}")
            for k, v in ciclo_2.items():
                print(f"Ciclo 2 - {k}: {round(v)}")
            for k, v in ciclo_3.items():
                print(f"Ciclo 3 - {k}: {round(v)}")

            qs_plantio = Plantio.objects.filter(safra__safra="2023/2024")
            total_plantado = qs_plantio.aggregate(Sum("area_colheita"))
            serializer_plantio = PlantioSerializer(qs_plantio, many=True)
    ...
    def update_plantio_from_farmBox(self, request, pk=None):
        ...
        try:
            result = process_farmbox_plantations_payload(request.data)
    ...
    @action(detail=False, methods=["GET"])
    def farmbox_sync_options(self, request):
        if not request.user.is_authenticated:
            return Response(
                {"message": "Você precisa estar logado!!!"},
                status=status.HTTP_400_BAD_REQUEST,
        ...
        safras = (
            Safra.objects
            .filter(id_farmbox__isnull=False, ativo=True)
            .order_by("-safra")
        )
        ...
        for safra in safras:
            state = FarmboxPlantioSyncState.objects.filter(safra=safra).first()

            dados.append({
                "id": safra.id,
                "safra": safra.safra,
                "id_farmbox": safra.id_farmbox,
                "last_sync_at": state.last_sync_at if state else None,
                "last_total_received": state.last_total_received if state else 0,
            })

        return Response({
            "msg": "Safras carregadas com sucesso.",
            "dados": dados,
        }, status=status.HTTP_200_OK)
    
    @action(detail=False, methods=["POST"])
    def sync_plantio_farmbox(self, request):
        if not request.user.is_authenticated:
            return Response(
                {"message": "Você precisa estar logado!!!"},
                status=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            if not safra_id:
                return Response(
                    {"message": "Informe a safra."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            safra = Safra.objects.filter(id=safra_id, ativo=True).first()
            ...
            if not safra.id_farmbox:
                return Response(
                    {"message": f"A safra {safra.safra} não possui id_farmbox cadastrado."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            state, _ = FarmboxPlantioSyncState.objects.get_or_create(safra=safra)

            updated_since_ms = None

            if not force_full:
                if updated_since:
                    updated_since_ms = get_milliseconds_from_datetime(updated_since)
                elif state.last_sync_at:
                    updated_since_ms = get_milliseconds_from_datetime(state.last_sync_at)

            farmbox_result = fetch_farmbox_plantations_parallel(
                harvest_id=safra.id_farmbox,
                updated_since_ms=updated_since_ms,
                max_workers=10,
            )

            data = farmbox_result["plantations"]
            farmbox_meta = farmbox_result["meta"]

            if dry_run:
                return Response({
                    "msg": "Busca realizada em modo teste. Nenhum dado foi gravado.",
                    "safra": safra.safra,
                    "id_farmbox": safra.id_farmbox,
                    "updated_since_ms": updated_since_ms,
                    "total_recebido": len(data),
                    "farmbox_meta": farmbox_meta,
                    "preview": data[:3],
                }, status=status.HTTP_200_OK)

            result = process_farmbox_plantations_payload(
                data,
            ...
            state.last_updated_since_ms = updated_since_ms
            state.last_total_received = len(data)
            state.last_result = {
                "farmbox_meta": farmbox_meta,
            ...
            return Response({
                "msg": "Sincronização Farmbox finalizada com sucesso.",
                "safra": safra.safra,
                "id_farmbox": safra.id_farmbox,
                "updated_since_ms": updated_since_ms,
                "total_recebido_farmbox": len(data),
                "farmbox_meta": farmbox_meta,

                # compatibilidade com log antigo
                "total_return": result.get("total_return", 0),
                "Total de Talhões alterados": result.get(
                    "Total de Talhões alterados",
                    result.get("alteradas_com_farmbox_update", 0),
                ),
                "criadas": result.get("criadas", 0),
                "atualizadas": result.get("atualizadas", 0),
                "ignoradas": result.get("ignoradas", 0),
            ...
            return Response(
                {"message": f"Ocorreu um erro ao sincronizar Farmbox: {e}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
                        ...
                        final_result[i["talhao__fazenda__nome"]].update(
                            {
                                i["talhao__id_talhao"]: {
                                    ...
                                    "inicializado_plantio": i["inicializado_plantio"],
                ...
                result = [
                    {
                        ...
                        "dados": {
                            ...
                            "data_plantio": i["data_plantio"],
                            "area_planejamento_plantio": i["area_planejamento_plantio"],
    ...
    def get_kmls_aviacao(self, request, pk=None):
        ...
        try:
            ...
            if parcelas_filter:
                ...
                try:
                    kml_content = create_kml(poligons_to_export, should_use_color)
                except Exception as e:
    ...
    def get_plot_mapa_data_fetchrn_app(self, request, pk=None):
        ...
        try:
            ...
            ).filter(ativo=True).filter(plantio_descontinuado=False)
...
def save_from_protheus_logic(data_json, user_id):
    # date_file = request.data["plantio"]
                ...
                current_safra = CicloAtual.objects.filter(nome="Colheita")[0]
                plantio_objs = {
                    (p.safra.safra, p.ciclo.ciclo, p.talhao.fazenda.id_d, p.talhao.id_talhao): p
                    for p in Plantio.objects.select_related('safra', 'ciclo', 'talhao', 'talhao__fazenda')
                    .filter(finalizado_plantio=True, finalizado_colheita=False, plantio_descontinuado=False, safra=current_safra.safra)
                }
                
                # deposito_query = Deposito.objects.all()
                deposito_objs = {d.pk: d for d in Deposito.objects.all()}
                succes = 0
                failed = 0
                problem = []
                success_list = []
                ...
                # Passo 1: preparar lista de possíveis duplicatas
                tickets_set = set()
                plantio_ids_cache = {}  # evita resolver o mesmo plantio várias vezes
                ...
                for item in data_json:
                    ...
                    for parcela in parcelas:
                        plantio = plantio_objs.get((safra, ciclo, origem_id, parcela))
                        if plantio:
                            plantio_id = plantio.id
                            plantio_ids_cache[(safra, ciclo, origem_id, parcela)] = plantio_id

                            final_ticket = f"{item['Filial']}{remove_leading_zeros(str(item['Ticket']))}"
                            romaneio = remove_leading_zeros(str(item["Num Romaneio"]))

                            tickets_set.add((final_ticket, plantio_id, romaneio))
                        else:
                            print(f"⚠️ Plantio não encontrado para {safra}/{ciclo} - {origem_id}/{parcela}")
                
                print('plantio id cache', plantio_ids_cache)
                # 2. Construir Q object para consulta em lote
                query = Q()
                for ticket, plantio_id, romaneio in tickets_set:
                    query |= Q(ticket=ticket, plantio_id=plantio_id, romaneio=romaneio)

                # 3. Fazer a consulta uma única vez
                existing_records = set(
                    Colheita.objects.filter(query).values_list("ticket", "plantio_id", "romaneio")
                )
                print('ecisting record', existing_records)
                        ...
                        if len(i["Placa do veiculo"]) > 0:
                            ...
                            if len(parcelas) > 1:
                                for index, parcela in enumerate(parcelas):
                                    print('get plantoid: ', str(safra), int(ciclo), int(origem_id), str(parcela))
                                    ...
                                    try:
                                        ...
                                        if plantio_id and deposito_id:
                                            try:
                                                ...
                                                if (final_ticket, plantio_id, romaneio) in existing_records:
                ...
                print('ids inseridos: ', id_farmtruck_list)
                if len(id_farmtruck_list) > 0:
                    headers = {
                        "Content-Type": "application/json",
...
class ColheitaPlantioExtratoAreaViewSet(viewsets.ModelViewSet):
    ...
    def update_colheita_area_from_farmbox(self, request, pk=None):
        if request.user.is_authenticated:
            ...
            filtered_query = Plantio.objects.filter(
                id_farmbox__in=list_of_ids
            )
            ...
            # Ordena cronologicamente para processar os apontamentos
            ...
            ignorados_duplicados_payload = 0
            erros = 0

            # ---------------------------------------------------------
            # Cache da soma REAL dos extratos existentes por plantio.
            #
            # Não usamos Plantio.area_parcial como fonte da verdade.
            # A fonte da verdade é ColheitaPlantioExtratoArea.
            # ---------------------------------------------------------
            ...
            # ---------------------------------------------------------
            # Evita processar duas vezes o MESMO apontamento dentro
            # do próprio payload recebido.
            #
            # A chave considerada é:
            # plantio + data + hora
            #
            # Essa é a mesma regra usada no banco para localizar
            # extrato existente.
            # ---------------------------------------------------------
            apontamentos_processados = set()
            ...
            for i in req_data_sorted:
                try:
                    ...
                    if not plantio_to_save:
                        raise ValueError(
                            f"Plantio with "
                            f"id_farmbox={plantio_id_to_save} "
                            f"not found"
                        )

                    area_to_save = Decimal(
                        str(
                            i["Area Aplicada"]
                        ).replace(",", ".")
                    )
                    ...
                    area_colheita = (
                        plantio_to_save.area_colheita
                        or Decimal("0")
                    )

                    # -------------------------------------------------
                    # Proteção contra duplicidade dentro do MESMO
                    # payload recebido.
                    # -------------------------------------------------
                    chave_apontamento = (
                        plantio_to_save.id,
                    ...
                    if chave_apontamento in apontamentos_processados:
                        ignorados_duplicados_payload += 1

                        print(
                            f"[IGNORADO DUPLICADO PAYLOAD] "
                            f"Plantio {plantio_to_save.id} / "
                            f"Farmbox {plantio_id_to_save}: "
                            f"data={data_to_save}, "
                            f"hora={hora_to_save}"
                        )
                    ...
                    if area_to_save <= Decimal("0"):
                        ...
                        area_parcial_final = min(
                            total_aplicado_to_save,
                            area_colheita
                        )

                        plantio_to_save.area_parcial = (
                            area_parcial_final
                        )
                        ...
                        print(
                            f"[IGNORADO AREA ZERO] "
                            f"Plantio "
                            f"{plantio_to_save.id} / "
                            f"Farmbox "
                            f"{plantio_id_to_save}: "
                            f"area_to_save="
                            f"{area_to_save}"
                        )
                    ...
                    # -------------------------------------------------
                    # Soma atual dos extratos do plantio.
                    # -------------------------------------------------
                    soma_extratos_atual = (
                        soma_extratos_por_plantio.get(
                            plantio_to_save.id,
                            Decimal("0")
                        )
                    )
                    ...
                    # MUITO IMPORTANTE:
                    #
                    # Descobre ANTES do cálculo se o apontamento
                    # já existe.
                    #
                    # A regra de não duplicidade continua:
                    #
                    # plantio + data + hora
                    #
                    # Se existir:
                    ...
                    if extrato_existente:
                        area_antiga = (
                            extrato_existente.area_colhida
                            or Decimal("0")
                        )
                    ...
                    # CORREÇÃO PRINCIPAL
                    #
                    # A soma atual contém o próprio extrato quando
                    # estamos atualizando um apontamento existente.
                    #
                    # Portanto precisamos removê-lo antes de calcular
                    # o saldo disponível.
                    # -------------------------------------------------
                    soma_outros_extratos = (
                        soma_extratos_atual
                        - area_antiga
                    )

                    if soma_outros_extratos < Decimal("0"):
                        soma_outros_extratos = Decimal("0")
                    ...
                    # -------------------------------------------------
                    # Se OUTROS extratos já preencheram toda a área,
                    # não existe espaço para este apontamento.
                    #
                    # Atenção:
                    # aqui não estamos considerando o próprio extrato
                    # existente, porque ele já foi removido acima.
                    # -------------------------------------------------
                    if saldo_disponivel <= Decimal("0"):
                        ignorados_sem_saldo += 1

                        soma_final_atual = min(
                            soma_extratos_atual,
                            area_colheita
                        )
                        ...
                        print(
                            f"[IGNORADO SEM SALDO] "
                            f"Plantio "
                            f"{plantio_to_save.id} / "
                            f"Farmbox "
                            f"{plantio_id_to_save}: "
                            f"soma_extratos_atual="
                            f"{soma_extratos_atual}, "
                            f"area_antiga="
                            f"{area_antiga}, "
                            f"soma_outros_extratos="
                            f"{soma_outros_extratos}, "
                            f"area_colheita="
                            f"{area_colheita}. "
                            f"Nenhum saldo disponível "
                            f"para este apontamento."
                        )
                    ...
                    #
                    # Por padrão salva exatamente o valor apontado
                    # pelo Farmbox.
                    # -------------------------------------------------
                    area_colhida_extrato = area_to_save

                    # -------------------------------------------------
                    # Só limita se:
                    #
                    # soma dos OUTROS extratos
                    # +
                    # apontamento atual
                    #
                    # ultrapassar a área total do plantio.
                    # -------------------------------------------------
                    if area_to_save > saldo_disponivel:
                        area_colhida_extrato = (
                            saldo_disponivel
                        )
                    ...
                    # -------------------------------------------------
                    # Salva/atualiza de forma atômica.
                    # -------------------------------------------------
                    with transaction.atomic():
                        ...
                        # -------------------------------------------------
                        # Como soma_outros_extratos já removeu a área
                        # antiga do registro atual, basta adicionar
                        # a nova área.
                        # -------------------------------------------------
                        soma_final_extratos = (
                            soma_outros_extratos
                        ...
                        # -------------------------------------------------
                        # area_parcial acompanha a soma REAL dos extratos.
                        # Nunca ultrapassa area_colheita.
                        # -------------------------------------------------
                    ...
                    if created:
                        ...
                        print(
                            f"{Fore.GREEN}"
                            f"Colheita Atualizada com sucesso!!"
                            f"{Style.RESET_ALL}"
                        )
                    ...
                    print(
                        ...
                        f"Area Salva={area_colhida_extrato}, "
                        f"Area Antiga={area_antiga}, "
                        f"Soma Outros={soma_outros_extratos}, "
                        f"Soma Final={soma_final_extratos}, "
                        f"Area Colheita={area_colheita}, "
                        f"created={created}"
                    )
                    ...
                    print(
                        ...
                        f"Problema em Salvar o Plantio: "
                        f"{i} \n"
                        f"{Fore.LIGHTRED_EX}"
                        f"Error: {e} "
                        f"{Style.RESET_ALL}"
            ...
            response = {
                ...
                "corrigidos_por_excesso_area_colheita": (
                    corrigidos_por_excesso
                ),
                "ignorados_por_regra_7_dias": (
                    ignorados_por_regra_7_dias
                ),
                "ignorados_sem_saldo": (
                    ignorados_sem_saldo
